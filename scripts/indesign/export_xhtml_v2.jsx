...
try {
  ...
  for (var i = 0; i < files.length; i++) {
    ...
    try {
      // Export as IDML (intermediate markup format)
      ...
      // Open the IDML file and replace missing fonts
      var doc = app.open(idmlFile);
      replaceMissingFonts(doc, standardFont);

      // Apply standard font to all text frames
      ...
      // Map paragraph styles to heading tags and process footnotes
  ...
  for (var i = 0; i < allFonts.length; i++) {
    try {
      ...
      if (font.status == undefined) {
        $.writeln("Replacing missing font: " + font.name + " with " + replacementFont);
...
function processFootnotes(xhtmlContent) {
  ...
  // Remove unnecessary file:// URL prefixes
